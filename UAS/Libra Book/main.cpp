#include "header.h"
using namespace std;
void masukkanBuku(), edit(), admin(), masuk(), member(), cari(int a);
...
void read(int kembalinya) {
    ...
    char pilihan;
    cout << "\nApakah anda ingin menggunakan fitur pencarian? (y/n) : ";
    cin >> pilihan;
    //cout << "balik? (y/n): "; cin >> balik;
    if ((pilihan == 'n' || pilihan == 'N') && kembalinya == 1) {
        admin();
    }
    else if ((pilihan == 'n' || pilihan == 'N') && kembalinya == 2) {
        member();
    }
...
void cari(int kembalinya) {
    ...
    if (file.is_open()) {
        ...
        while (getline(file, buku_yang_dicari.kode, ',')) {
            ...
            getline(file, buku_yang_dicari.tersedia);
